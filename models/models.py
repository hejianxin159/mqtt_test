...
class Camera(Base):
    ...
    project_id = Column(String(32), default=uuid.uuid4().hex, nullable=True)
...
class PersonResource(Base):
    ...
    attachment_id = Column(String(300))                 # 图片
...
class PersonSync(Base):
    ...
    business_id = Column(String(50), nullable=True)     # 业务编号
    person_id = Column(String(18), nullable=True)       # 人员代码编号（18位：T+TIME+RANDOM)
    camera_id = Column(String(50), nullable=True)       # 下发的相机
    sync_status = Column(SmallInteger, default=0)       # 操作状态 0: 未操作; 1: 操作成功; 2: 操作失败
    sync_desc = Column(String(400), nullable=True)
    attachment_id = Column(String(300))                 # 图片
    from_date = Column(DateTime, server_default=func.now())
    update_date = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
if __name__ == '__main__':
    ...
    # camera1 = Camera(name='test01', sn="75b994c0-578b65a4", status=0, online=0, direction="IN")
    # camera2 = Camera(name='test02', sn="75b994c0-578b65a4", status=0, online=0, direction="OUT")
    # db_session.add(camera1)
    # db_session.add(camera2)

    person1 = PersonResource(name="hejx", )
    picture = Attachment(name="hejx", url="http://10.28.25.213:8080/IMG_1227.jpeg")
